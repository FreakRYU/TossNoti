...
평소 쓰던 핸드폰이 인스타그램 알고리즘 판정으로 부당하게 차단되면서 시작된 프로젝트입니다. 계정이 막힌 게 아니라 **기기 자체가 식별되어 차단**되는 케이스라, 같은 폰에서는 새 계정을 만들어도 곧바로 다시 잠겼고, 이의신청도 무력했어요.

해결책은 단순했습니다 — 인스타그램이 한 번도 실행된 적 없는 **다른 기기(이 경우 태블릿)에서 인스타를 사용**하기. 다만 그러면 새로운 문제가 생깁니다: **인스타 알림을 보려면 매번 태블릿을 직접 들여다봐야 한다는 점**. 메인 폰에는 다시 깔 수 없으니까요.

그래서 만들었습니다 — **태블릿에 도착한 알림을 메인 폰으로 즉시 토스해주는 릴레이**. 인스타뿐만 아니라 **카톡, 텔레그램, 메일 등 송신 기기에 깔린 어떤 앱의 알림이든** PIN으로 페어링된 폰으로 전달됩니다.

비슷한 상황(특정 앱이 기기 단위로 막혔거나, 작업용 태블릿과 메인 폰을 따로 쓰는 경우)에 누구나 쓸 수 있도록 일반화했습니다.
...
초기 아이디어부터 완성까지 **약 5시간** 소요. 진행 방식:

1. **아이디어 정리** — 페어링 방식(PIN), 전송 채널(ntfy.sh), 양쪽 기기 역할 분리(`NotificationListenerService` + Foreground Service) 등 핵심 아키텍처 스케치.

2. **Google AI Studio (Gemini 3.5 Flash)** — 위 아이디어를 입력해 안드로이드 프로젝트 골격을 빠르게 생성. UI 기본 틀, 권한 처리, 메시지 송수신 뼈대 코드를 자동으로 잡아냄.

3. **Claude Code (Opus 4.7)에서 반복 개선** — 생성된 프로젝트를 로컬로 모두 내려받은 뒤, 다음과 같은 영역을 단계별로 코드 리뷰 + 수정 반복:
   ...
   ...등 **30여 개의 개별 개선**이 차례로 적용됨.

AI 생성 코드의 **빠른 골격 잡기**와 사람이 운전하는 **방향성 + 품질 검수**가 결합된 워크플로우. 처음부터 직접 짰다면 며칠 걸렸을 작업을 한나절 안에 마쳤습니다.
