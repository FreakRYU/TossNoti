...
│                                                                    │
│  Source Apps          ┌───────────────────────────────────┐        │
│  (Instagram /         │ AlarmListenerService              │        │
│   KakaoTalk /  ─OS콜백→│ · 송신 모드 + 대상 앱 필터          │        │
│   Telegram …)         │ · Dedup 캐시 (5초 TTL)            │        │
│                       └─────────────────┬─────────────────┘        │
│                                          ↓                          │
│                       ┌───────────────────────────────────┐        │
│                       │ AppMetadataHelper                 │        │
│                       │   loadLabel(pkg) → "Instagram"    │        │
│                       └─────────────────┬─────────────────┘        │
│                                          ↓                          │
│  SharedPreferences ┄┄┄→┌───────────────────────────────────┐        │
│  · sender_pin          │ RelaySender                       │        │
│  · target_apps         │ · JSON 페이로드 구성               │        │
│  · is_sender_mode      │ · OkHttp HTTPS POST              │ ─→ Room │
│                        └─────────────────┬─────────────────┘   DB  │
│                                          ↓             (sent logs)  │
│                       ┌───────────────────────────────────┐        │
│                       │ CryptoUtils                       │        │
│                       │   PIN ┬─ SHA-256 ──→ 토픽 이름      │        │
│                       │       └─ PBKDF2  ──→ AES-256 키    │        │
│                       │   AES-GCM encrypt (랜덤 IV + tag)  │        │
│                       └─────────────────┬─────────────────┘        │
└──────────────────────────────────────────│─────────────────────────┘
                                           │ HTTPS POST
                                           │ body = base64(IV ‖ ct ‖ tag)
                                           ▼
   ╔════════════════════════════════════════════════════════════════╗
   ║          ntfy.sh  —  퍼블릭 메시지 릴레이 (계정 불필요)            ║
   ║                                                                ║
   ║   · 토픽: atoss_<24 hex>  (SHA-256(salt|PIN)에서 파생)            ║
   ║   · 12시간 메시지 보관                                            ║
   ║   · 익명 publish + subscribe                                    ║
   ║   · 본문은 암호문이라 운영자도 내용 못 봄 (E2E)                     ║
   ╚════════════════════════════════════════════════════════════════╝
                                           │ HTTPS GET (long-poll)
                                           │ /<topic>/json?since=<last_id>
                                           ▼
┌──────────────────────────── 수신 폰 ──────────────────────────────┐
│                                                                   │
│  ┌──────────────────────┐                                         │
│  │ ScreenStateReceiver  │     ┌───────────────────────────────┐   │
│  │ SCREEN_OFF → 스트림   │ ───→│ AlarmReceiverService          │   │
│  │   끊고 모뎀 sleep      │ pause│ (Foreground Service)          │   │
│  │ SCREEN_ON + 10초 →   │ /res │ · Long-poll JSON 스트림        │   │
│  │   재연결 (since 커서) │ ume  │ · 지수 백오프 3→30초 재연결    │   │
│  └──────────────────────┘     │ · since=<id> 누락 메시지 복구  │   │
│                                └─────────────┬─────────────────┘   │
│                                               ↓                    │
│  SharedPreferences ┄┄┄→┌───────────────────────────────────┐      │
│  · receiver_pin        │ CryptoUtils                       │      │
│  · last_message_id     │ · 같은 PIN → 같은 AES 키            │      │
│                        │ · AES-GCM decrypt + tag 검증       │      │
│                        │   (검증 실패 시 메시지 드롭)         │      │
│                        └─────────────────┬─────────────────┘      │
│                                          ↓                         │
│                       ┌───────────────────────────────────┐        │
│                       │ NotificationManager               │ ─→ Room│
│                       │   "[appLabel] title"               │   DB   │
│                       │   본문 (BigTextStyle)              │(received│
│                       └───────────────────────────────────┘ logs)  │
│                                                                   │
└───────────────────────────────────────────────────────────────────┘
```
